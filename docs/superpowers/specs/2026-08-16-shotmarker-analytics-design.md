...
- 复用 `zhangrh.shop` 的 Nginx 单一 JSONL 和查询能力。
...
- 不为 ShotMarker 单独增加日志轮转、定时清理或固定保留窗口。
...
- Nginx 负责接收、生成服务端接收时间并追加写入单一 JSONL。
- Backend 负责校验、过滤和聚合，不修改原始日志。
- 服务器运维负责低频检查文件大小，并在 `32 MiB` 阈值触发存储方案重新设计。
...
- 原始埋点当前写入单一日志，没有固定自动过期时间；文件达到 `32 MiB` 时重新评估
  轮转、归档、删除或数据库方案。
