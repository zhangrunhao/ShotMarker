...
- 最后更新时间：2026-08-16
- 基准分支：`main`
- 基准提交：`af44535 fix: 取消集锦任务后不再上报成功`
- 基准工作区：更新本文档前无未提交改动
...
当前阶段是下一次发布前的观测与隐私准备：GlitchTip 崩溃/错误上报已经接入并验证；ShotMarker 最小埋点客户端、Privacy Manifest、`zhangrh.shop` 的 ShotMarker 汇总查询支持以及公开隐私政策源文件已经完成并通过本地验证。生产部署、真实 Release 上报和线上政策页面仍未在本次任务中验证，后续再统一执行真机验证、Archive、dSYM 上传和 TestFlight 发布。
...
ShotMarker 客户端、四个调用点、运行策略、Privacy Manifest，以及 `zhangrh.shop` 的查询兼容、协议文档和隐私政策源文件均已完成。代码与本地构建已经验证，但服务端部署、真实数据落盘、线上汇总查询和 Release 客户端端到端结果仍未验证。

### 4.2 隐私与商店声明

- `zhangrh.shop` 的 ShotMarker 隐私政策源文件已覆盖第一方产品分析的客户端字段、服务器附加字段、用途、排除项和保留边界；线上页面是否已部署未验证。
...
- Frontend 测试：111/111 通过。
- Backend 测试：26/26 通过。
- Frontend lint 与 typecheck：通过。
- Hub、Cardgame、ShotMarker 和 Analytics 构建：通过。

该结果只证明仓库代码与构建通过，不证明网站、Nginx 或 Backend 已部署，也没有在生产环境写入测试事件。

### 5.4 尚未执行的验证

- 最新代码的真实 iPhone 与 Apple Watch 联调。
- TestFlight Release 构建的真实埋点上报。
- `zhangrh.shop` 生产 `/track` 数据落盘、ShotMarker 汇总查询和线上隐私政策页面。
- 不连接调试器的真机受控崩溃。
...
- 埋点服务端、汇总查询与隐私政策的仓库代码已验证，但生产部署属于变化中的外部状态，本次未验证；不能据此判断线上链路已经完成。
- GlitchTip 已验证错误事件接收，但尚未用正式 Release 真机崩溃验证原生崩溃缓存、下次启动发送和符号化。
- 正式发布若不上传匹配 dSYM，原生崩溃堆栈可能只有地址而缺少函数名与源码位置。
- 公开隐私政策源文件已经更新，但线上页面未验证；App Store Connect 数据声明仍需在包含 GlitchTip 和埋点的版本发布前更新。
...
1. 部署 `zhangrh.shop` 的埋点与隐私政策变更，受控验证数据落盘、ShotMarker 聚合查询和原始标识脱敏。
