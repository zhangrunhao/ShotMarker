...
- 第一版继续使用完整语音转写后解析口令的路线，不引入自定义关键词模型。
- 用户停止录制并选择本地视频后才开始分析；不要求训练过程中实时出结果。
- 领夹麦音频必须直接保存在训练视频音轨中。
...
- 不做实时比分直播。
- 不开发 ShotMarker 自有相机。
- 不使用 Sound Analysis、Create ML 或 Core ML 训练专用关键词检测模型。
...
5. 在真机性能数据证明完整转写不能满足产品等待时间后，再评估自定义关键词模型。
6. 罚球、助攻、抢断和犯规等更多统计口令。
7. 针对不同事件类型使用不同剪辑窗口。
8. 在集锦中叠加球员姓名、分值和比分字幕。
